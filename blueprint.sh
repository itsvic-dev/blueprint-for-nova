...
# Blueprint for Nova © 2024-2026 it's vic!
...
depend() {
  ...
  if [[ $DEPEND_MISSING == true ]]; then
    ...
    if ! [ "$(ls "node_modules/"*"vite"* 2> /dev/null)"    ]; then PRINT FATAL "Missing dependency \"vite\". Forgot to run 'yarn install'?"; fi
